...
a new project as described `below <#installation>`_.  You'd have two
example object types (``Person`` and ``Post``) to play with.
(As you read this section, if you find yourself wondering things like
how the types and their schemas are defined, you may want to jump ahead to the `Usage`_ section.)
...
The object has been persisted in MongoDB and now has an ObjectId, creation and modification timestamps and an Etag. Let's check the children of the ``People``
collection again::
...
Ooh... ``400 Bad Request`` and an error message in the body with the reason.
...
Now let's upload a photo for Audrey Horne::
...
Let's update Audrey Horne's record with the new photo file::
    ...
    {"error": "Requests must supply both If-Unmodified-Since and If-Match headers."}

What's going on here?  Audrey implements `optimistic concurrency control <http://en.wikipedia.org/wiki/Optimistic_concurrency_control>`_ in an effort to avoid silent data loss.  PUT requests to update an existing resource and DELETE requests to remove an existing resource must include "If-Unmodified-Since" and "If-Match" headers whose values should match the "Last-Modified" and "Etag" headers from the response to a GET of that same resource.  Let's examine the response headers to get those two values::
...
.. FIXME - update line nums based on new resources.py
...
Note on line FIXME that this Post type has a reference named ``author``
